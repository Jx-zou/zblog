...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public Page<List<ArticleVo>> search(Integer size, Integer offset, String search) {
        Long total;
        String articleTotalName = CommonContent.REDIS_NAME_ARTICLE_TOTAL;
        ...
        //查询redis中是否有文章
        String articlePageName = CommonContent.REDIS_NAME_ARTICLE_PAGE;
        if (StringUtils.isBlank(search) && Boolean.TRUE.equals(businessRedisTemplate.hasKey(articlePageName))) {
            if (Boolean.TRUE.equals(businessRedisTemplate.opsForHash().hasKey(articlePageName, pageName(offset, size)))){
                return new Page<>(offset, size, total, (List<ArticleVo>) businessRedisTemplate.opsForHash().get(articlePageName, pageName(offset, size)));
            }
        }
        ...
        businessRedisTemplate.opsForHash().put(articlePageName, pageName(offset, size), articleVos);
