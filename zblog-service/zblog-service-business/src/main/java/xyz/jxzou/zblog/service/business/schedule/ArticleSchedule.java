...
public class ArticleSchedule {
    ...
    private void articleCache() {
        ...
        businessRedisTemplate.opsForValue().set(CommonContent.REDIS_NAME_ARTICLE_PAGE, count);
        for (int offset = 0; offset < count / size; offset++) {
            List<ArticleVo> articleVos = articleMapper.search(offset, size, null);
            businessRedisTemplate.opsForHash().put(CommonContent.REDIS_NAME_ARTICLE_PAGE, obtainName(offset, size), articleVos);
        }
        businessRedisTemplate.expire(CommonContent.REDIS_NAME_ARTICLE_PAGE, 1L, TimeUnit.HOURS);
    }
