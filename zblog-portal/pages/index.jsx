  ...
  const local = useCallback(() => {
    if (CookieUtils.has(COOKIE_NAMES.PUBLICKEY)) {
      ...
      if (CookieUtils.has(COOKIE_NAMES.TOKEN) && CookieUtils.has(COOKIE_NAMES.USERINFO)) {
        refreshToken()
        refreshUserinfo()
        dispatch(changeAuth({ cid: CookieUtils.get(COOKIE_NAMES.TOKEN), pkey: CookieUtils.get(COOKIE_NAMES.PUBLICKEY) }))
      ...
      if (CookieUtils.has(COOKIE_NAMES.CLIENTID)) {
        refreshClientId()
        dispatch(changeAuth({ cid: CookieUtils.get(COOKIE_NAMES.CLIENTID), pkey: CookieUtils.get(COOKIE_NAMES.PUBLICKEY) }))
        return true
      }
